...
def createTupleIntentRep(rowIDs, sessQuery, configDict):
    ...
    if rowIDs is None:
        rowIDs = qp.fetchRowIDs(sessQuery, configDict)
    if rowIDs is None:
        ...
        with open(configDict['QUERYSESSIONS']) as f:
            for line in f:
                ...
                for i in range(1, len(sessQueries) - 1):  # we need to ignore the empty query coming from the end of line semicolon ;
                    ...
                    resObj = createTupleIntentRep(None, sessQuery, configDict) #rowIDs passed should be None, else it won't fill up
                    queryName = sessQuery+"Query"+str(i)
                    outputIntentLine = queryName+";"+resObj
