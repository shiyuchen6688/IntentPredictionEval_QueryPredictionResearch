...
def createSQLString(intentObj):
    ...
    if intentObj.selPredOps is not None and len(intentObj.selPredOps) > 0:
        actualSQLStr += "SEL PRED Ops: " + str(intentObj.selPredOps) + "\n"
    if intentObj.selPredColRangeBins is not None and len(intentObj.selPredColRangeBins) > 0:
...
def fixSelPredColRangeBin(selCol, intentObj, precOrRecallFavor):
    ...
    maxColRangeBinIndex, maxWeightVal = max(enumerate(intentObj.predictedY[colRangeBinStartIndex:colRangeBinEndIndex+1]),
