...
class SchemaDictsOpConst(SchemaDicts):
    def __init__(self, tableDict, tableOrderDict, colDict, joinPredDict, joinPredBitPosDict, selPredOpBitPosDict, selPredColRangeBins, selPredColRangeBitPosDict):
        SchemaDicts.__init__(self, tableDict, tableOrderDict, colDict, joinPredDict, joinPredBitPosDict)
        self.selPredOpBitPosDict = selPredOpBitPosDict
        self.selPredColRangeBins = selPredColRangeBins
        self.selPredColRangeBitPosDict = selPredColRangeBitPosDict
        self.selPredOpStartBitIndex = self.joinPredicatesStartBitIndex + self.joinPredicatesBitMapSize
        self.selPredOpBitMapSize = estimateSelPredOpBitMapSize(self)
        self.selPredColRangeBinStartIndex = self.selPredOpStartBitIndex + self.selPredOpBitMapSize
        self.selPredColRangeBinSize = estimateSelPredColRangeBinSize(self)
        self.allOpSize = self.queryTypeBitMapSize + self.tableBitMapSize + self.allColumnsSize * 10 \
                         + self.limitBitMapSize + self.joinPredicatesBitMapSize + self.selPredOpBitMapSize + \
                         self.selPredColRangeBinSize
...
def populateJoinPreds(schemaDicts):
    ...
    for tablePairIndex in schemaDicts.joinPredBitPosDict:
        ...
        for indexToSet in range(startBitPos, endBitPos): # not endPos+1 as one dimension is extra empty here
...
def populateSelPredColRangeBins(schemaDicts):
    opString = "selPredColRangeBin"
    for colName in schemaDicts.selPredColRangeBins:
        startEndBitPos = schemaDicts.selPredColRangeBitPosDict[colName]
        startBitPos = startEndBitPos[0] + schemaDicts.selPredColRangeBinStartIndex
        endBitPos = startEndBitPos[1] + schemaDicts.selPredColRangeBinStartIndex
        for indexToSet in range(startBitPos, endBitPos + 1):  # endPos+1 because we want to go from start to endBitPos
...
def populateSelPredOps(schemaDicts):
    opString = "selPredOp"
    selOps = ['eq', 'neq', 'leq', 'geq', 'lt', 'gt', 'LIKE']
    for colName in schemaDicts.selPredOpBitPosDict:
        startEndBitPos = schemaDicts.selPredOpBitPosDict[colName]
        startBitPos = startEndBitPos[0] + schemaDicts.selPredOpStartBitIndex
        endBitPos = startEndBitPos[1] + schemaDicts.selPredOpStartBitIndex
        for indexToSet in range(startBitPos, endBitPos+1): # endPos+1 because we want to go from start to endBitPos
...
def populateBiDirectionalLookupMap(schemaDicts):
    ...
    schemaDicts = populateSelPredOps(schemaDicts)
...
def estimateSelPredOpBitMapSize(schemaDicts):
    selPredOpBitPosDict = schemaDicts.selPredOpBitPosDict
    selPredOpBitCount = 0
    for selCol in selPredOpBitPosDict:
        selPredOpBitCount += selPredOpBitPosDict[selCol][1] - selPredOpBitPosDict[selCol][0] + 1
    return selPredOpBitCount
...
def refineIntentForQuery(threadID, predictedY, topKCandidateVector, schemaDicts, precOrRecallFavor, configDict, curIntentBitVec):
...
def refineIntentForTable(threadID, predictedY, topKCandidateVector, schemaDicts, precOrRecallFavor, configDict, curIntentBitVec):
    predictedIntentObj = CreateSQLFromIntentVec_selOpConst.regenerateSQLTable(predictedY, topKCandidateVector, curIntentBitVec, schemaDicts, configDict)
    return predictedIntentObj

def refineIntent(threadID, predictedY, topKCandidateVector, schemaDicts, precOrRecallFavor, configDict, curIntentBitVec):
    ...
    if configDict['RNN_PREDICT_QUERY_OR_TABLE'] == 'QUERY':
        intentObj = refineIntentForQuery(threadID, predictedY, topKCandidateVector, schemaDicts, precOrRecallFavor, configDict, curIntentBitVec)
    elif configDict['RNN_PREDICT_QUERY_OR_TABLE'] =='TABLE':
        intentObj = refineIntentForTable(threadID, predictedY, topKCandidateVector, schemaDicts, precOrRecallFavor, configDict, curIntentBitVec)
...
def predictTopKNovelIntentsSingleThread(threadID, predictedY, schemaDicts, configDict, curIntentBitVec):
    ...
    for threshold in thresholds:
        ...
        if int(configDict['TOP_K']) == 3 and threshold == 0.8:
            ...
            precOrRecallFavor = "recall"
        topKNovelIntent = refineIntent(threadID, predictedY, topKCandidateVector, schemaDicts, precOrRecallFavor, configDict, curIntentBitVec)
...
def createAndRefineIntentsForMockQueries(schemaDicts, configDict):
    intentObj = CreateSQLFromIntentVec_selOpConst.SQLForBitMapIntent(schemaDicts, None, None, None)
    ...
    if intentObj.queryType is not None:
        ...
        CreateSQLFromIntentVec_selOpConst.setBit(bitToSet, intentObj)
    ...
    for joinPred in intentObj.joinPreds:
        ...
        CreateSQLFromIntentVec_selOpConst.setBit(bitToSet, intentObj)
    #intentObj.selPredOps = []
    intentObj.selPredOps = []
    for selPredOp in intentObj.selPredOps:
        bitToSet = schemaDicts.backwardMapOpsToBits[selPredOp + ";selPredOp"]
        CreateSQLFromIntentVec_selOpConst.setBit(bitToSet, intentObj)
    #intentObj.selPredColRangeBins = []
    for selPredColRangeBin in intentObj.selPredColRangeBins:
        bitToSet = schemaDicts.backwardMapOpsToBits[selPredColRangeBin + ";selPredColRangeBin"]
        CreateSQLFromIntentVec_selOpConst.setBit(bitToSet, intentObj)
    refineIntentForQuery(0, None, intentObj.intentBitVec, schemaDicts, "recall", configDict, None)
...
if __name__ == "__main__":
    ...
    schemaDicts = readSchemaDicts(configDict)
    #intentObjDict = CreateSQLFromIntentVec_selOpConst.readIntentObjectsFromFile("/Users/postgres/Documents/DataExploration-Research/MINC/InputOutput/tempVector")
