...
if __name__ == "__main__":
    with open('Abt-Buy-numAtoms.csv') as f:
        lines = f.readlines()
        header = lines[0]
    ...
    plt.plot(cols[tokens[5]], cols[tokens[0]], 'r', linewidth = 4, marker = '^', markevery=50, markersize = 12, label= 'Trees(2)')
    plt.plot(cols[tokens[5]], cols[tokens[1]], 'b', linewidth = 4, marker = '*', markevery=50, markersize = 12, label= 'Trees(10)')
    plt.plot(cols[tokens[5]], cols[tokens[2]], 'g', linewidth = 4, marker = 'o', markevery=50, markersize = 12, label= 'Trees(20)')
    ...
    plt.xlabel('#Labeled Examples',fontsize=38)
    #plt.yticks(np.arange(0, 1.1, 0.1))
    plt.yscale('log')
    plt.ylabel('# DNF Atoms (log-scale)',fontsize=38)
    plt.title("#DNF Atoms vs. #Labels\nAbt-Buy", fontsize=38)
    plt.legend(loc = 'lower right', prop={'size':38}) 
    #plt.plot([1,2,3,4],[5,6,7,8], 'ro')
    #plt.savefig('Abt-Buy-numAtoms.png')
    plt.show()
