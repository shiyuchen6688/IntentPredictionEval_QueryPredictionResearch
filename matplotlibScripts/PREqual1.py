...
if __name__ == "__main__":
    ...
    PROne = [12859, 7190, 778, 122, 2] 
    PRNotOne = [991, 2273, 3173, 3129, 1639] 
    PNotR = [593, 4980, 10492, 11192, 12802]
    Types = 5
    barwidth = 1.0/(Types+2)
    
    r1 = np.arange(len(PROne))
    r2 = [x+barwidth for x in r1]
    r3 = [x+barwidth for x in r2]
    fig, ax = plt.subplots()
    rects1 = ax.bar(r1, PROne, color='deepskyblue', width = barwidth, edgecolor='black', label='P=R=1.0', hatch='*') 
    rects2 = ax.bar(r2, PRNotOne, color='lawngreen', width = barwidth, edgecolor='black', label='P=R!=1.0', hatch = 'O') 
    rects3 = ax.bar(r3, PNotR, color='lightcoral', width = barwidth, edgecolor='black', label='P!=R', hatch='x') 

    ax.set_title('Precision Recall Equivalence\n(Course Website)', fontsize=22)
    ax.set_xticks(r1+barwidth)
    ax.set_xticklabels(('Q-Learn', 'RNN-S', 'CF-SVD', 'CF-Cos', 'RNN-H'), fontsize=25)
    #ax.set_yscale('log')
    plt.ylabel('#Queries',fontsize=20)
