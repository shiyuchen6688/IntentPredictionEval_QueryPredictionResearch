...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(r1, rnn_fscore, color='lightskyblue', width = barwidth, edgecolor='black', label='RNN-S', hatch='*') 
    rects2 = ax.bar(r2, lstm_fscore, color='orange', width = barwidth, edgecolor='black', label='LSTM-S', hatch = 'O') 
    rects3 = ax.bar(r3, gru_fscore, color='gold', width = barwidth, edgecolor='black', label='GRU-S', hatch='x') 

    ax.set_title('Effect of Session Context\nFragment Quality(Bus Tracker)', fontsize=24)
