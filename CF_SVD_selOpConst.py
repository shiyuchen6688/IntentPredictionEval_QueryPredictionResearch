...
class SVD_Obj:
    def __init__(self, configDict):
        ...
        self.queryVocab = {}  # key is SHA hexDigest of the bitmap and val is sessID,queryID
        self.sessAdjList = {} # key is sess index and val is a set of query vocab SHA hexDigests
...
def createMatrix(svdObj):
    ...
    for sessID in svdObj.sortedSessKeys:
        sessQueryIDs = svdObj.sessAdjList[sessID]
        rowEntry = []
        for queryVocabID in svdObj.queryVocab.keys():
            sessQueryID = svdObj.queryVocab[queryVocabID]
            if sessQueryID in sessQueryIDs:
                rowEntry.append(1.0)
            else:
                rowEntry.append(0.0)
